...
import type { Condition, ConditionGroup, ExportHistoryRecord, LifecycleGroupId, OverallAdDataRow, PerformanceRow, RuleAction, RuleRunHistoryRecord } from "@/lib/types";
...
function buildBlockedIdentityId(campaignName: string, adGroupName: string) {
...
function containsScope(container: string[], scope: string[]) {
  return scope.every((id) => container.includes(id));
}

function countMatchedOverallKeywords(rows: OverallAdDataRow[]) {
  return new Set(
    rows
...
async function readOverallImportFile(file: File): Promise<OverallImportFile> {
...
function lifecycleGroupLabel(lifecycleGroupId: LifecycleGroupId) {
...
function summarizeCondition(condition: Condition) {
...
function flattenConditionGroup(group: ConditionGroup): Condition[] {
  return group.conditions.flatMap((item) => "logic" in item ? flattenConditionGroup(item) : [item]);
}

function summarizeAction(action: RuleAction) {
  switch (action.type) {
...
async function downloadGroupingStatusWorkbook(rows: Array<Array<string | number | undefined>>) {
...
async function readGroupingStatusFile(file: File) {
