        ...
        <div className="thin-scrollbar flex-1 overflow-auto">
          <section className="border-b border-border bg-surface-muted px-5 py-4">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8">
              ...
              <ProgressField label="预估售价 USD">
                <input className={inputClass} type="number" min="0" step="0.01" value={draft.forecastPrice || ""} onChange={(event) => setNumber("forecastPrice", event.target.value)} />
              </ProgressField>
            </div>
            <div className="mt-3 flex items-center justify-end gap-2 text-sm">
              <span className="text-muted">预估月销售额</span>
              <strong className="metric-tabular text-lg text-foreground">
                      ...
                      <tr key={definition.id} className="border-t border-border align-top">
                        ...
                        <td className="w-32 px-2 py-2">
                          <select className={inputClass} value={stage.status} onChange={(event) => updateStageStatus(index, event.target.value as ProductOperationStageStatus)}>
                            {operationStageStatusOptions.map((option) => (
                        ...
                        <td className="w-36 px-2 py-2">
                          <input className={inputClass} value={stage.owner} onChange={(event) => updateStage(index, { owner: event.target.value })} />
                        </td>
          ...
          <section className="border-t border-border px-5 py-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-foreground">变更记录</h3>
            </div>
