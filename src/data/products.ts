...
const productSeeds: ProductSeed[] = [
  { chineseName: "原木色桌面理线架", englishName: "Wooden Cable Organizer Tray", keywords: "desk organizer,cable tray,wood shelf", specs: "原木色;43*30*0.8cm", hsCode: "4421999090", purchasePrice: 5.8, productWeightG: 430, productSizeCm: { length: 43, width: 30, height: 0.8 } },
...
export const initialProducts: Product[] = productSeeds.map((seed, index) => {
  ...
  return {
    ...
    asin: index % 4 === 1 ? "" : `B0${String(10000000 + index * 7391).slice(0, 8)}`,
    developer: developers[index % developers.length],
    purchasePrice: seed.purchasePrice,
    status: statuses[index % statuses.length],
    supplierName: suppliers[index % suppliers.length],
    supplierUrl: `https://example.com/supplier/${sku}`,
    specs: seed.specs,
    purchaseLeadTime: leadTimes[index % leadTimes.length],
    createdAt: `2026-${String(6 + Math.floor(index / 30)).padStart(2, "0")}-${String(1 + (index % 28)).padStart(2, "0")}`,
    keywords: seed.keywords,
    note: index % 5 === 4 ? "注意检查外观专利与关键词合规。" : "演示数据，可在详情页继续补充采购和上架信息。",
    cancelReason: statuses[index % statuses.length] === "canceled" ? "演示数据：供应商报价或合规风险不符合继续开发要求。" : "",
    hsCode: seed.hsCode,
    images: [],
    competitorAsins: [`B0${String(20000000 + index * 5179).slice(0, 8)}`, `B0${String(30000000 + index * 6833).slice(0, 8)}`],
    productWeightG: seed.productWeightG,
    packageWeightG: seed.productWeightG + 90 + (index % 6) * 25,
