...
function mergeProtectedAccounts(
  ...
  const preservedAccounts = protectedAccounts.map((account) => ({
    ...stripAccountPassword(lockDefaultSuperAccount(account, defaultSuperAccountIds)),
    organizationId,
    sortOrder: 0,
  }));
...
export async function GET(request: Request) {
  try {
    ...
    const defaultSuperAccountIds = new Set(
      userMemberships
        .filter((membership) => isBootstrapAdminEmail(membership.user.email))
    ...
    const result = await runRosterSaveTransaction(() =>
      prisma.$transaction(
        async (tx) => {
          ...
          const currentMembers = (await tx.teamRosterMember.findMany({
            where: {
              organizationId: user.organizationId,
            },
            orderBy: {
