...
import { productAgentId } from "@/lib/agent-platform/product";
...
export async function POST(request: Request) {
  try {
    ...
    const opportunity = isObject(body.opportunity) ? body.opportunity : report && isObject(report.sourceOpportunity) ? report.sourceOpportunity : null;
    const decisionAt = new Date();

    if (!report && !opportunity) {
      return NextResponse.json({ error: "A product report or market opportunity is required." }, { status: 400 });
    }

    const reportProjectDraft = report && isObject(report.projectDraft) ? report.projectDraft : null;
    const reportScores = report && isObject(report.scores) ? report.scores : null;
    const projectName = stringValue(
      (reportProjectDraft && typeof reportProjectDraft.projectName === "string" ? reportProjectDraft.projectName : undefined) ??
        (opportunity && typeof opportunity.productIdea === "string" ? opportunity.productIdea : undefined) ??
        "Product Project",
    );
    const executionId = `execution-${productAgentId}-project-${decisionAt.getTime()}`;
    const approval = createApprovalRequest({
      executionId,
      riskLevel: "HIGH",
      recommendation: {
        summary: `Create product project for ${projectName}.`,
        evidence: report?.evidence ?? opportunity?.evidence ?? [],
        risks: ["Product project creation is approval gated."],
        confidence: numberValue(
          reportScores && typeof reportScores.executionConfidence === "number" ? reportScores.executionConfidence : undefined,
          numberValue(opportunity && typeof opportunity.confidence === "number" ? opportunity.confidence : undefined, 0.7),
        ),
      },
      action: {
        type: "project.create",
        target: String(opportunity?.opportunityId ?? projectName),
        payload: {
    ...
    const execution = {
      ...
      agentDefinitionId: productAgentId,
      requestedByUserId: user.id,
      ...
      decision: {
        ...
        rationale: "The product project must be reviewed before it becomes an executable project.",
    ...
    const traces = [
      createTraceEvent({
        ...
        sequence: 1,
        type: "decision",
        message: "Product project draft prepared",
        payload: {
          opportunity,
          projectName,
        } as AgentJsonValue,
      ...
      createTraceEvent({
        ...
        sequence: 2,
        type: "recommendation",
        message: "Product project approval requested",
    ...
    const transactionResults = await prisma.$transaction([
      ...
      prisma.agentTask.create({
        data: {
          ...
          title: `Create product project for ${projectName}`,
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to create product project approval." },
      { status: 500 },
    );
  }
}
