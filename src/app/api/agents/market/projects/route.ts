...
import { marketAgentId } from "@/lib/agent-platform/market";
...
export async function POST(request: Request) {
  try {
    ...
    const opportunity = isObject(body.opportunity) ? body.opportunity : null;
    const report = isObject(body.report) ? body.report : null;
    const decisionAt = new Date();

    if (!opportunity) {
      return NextResponse.json({ error: "A market opportunity is required." }, { status: 400 });
    }

    const opportunityId = typeof opportunity.opportunityId === "string" ? opportunity.opportunityId : `opportunity-${decisionAt.getTime()}`;
    const executionId = `execution-${marketAgentId}-project-${decisionAt.getTime()}`;
    const approval = createApprovalRequest({
      executionId,
      riskLevel: "HIGH",
      recommendation: {
        summary: `Create project for ${stringValue(opportunity.productIdea, "selected market opportunity")}.`,
        evidence: isObject(opportunity.evidence) || Array.isArray(opportunity.evidence) ? opportunity.evidence : [],
        risks: ["Project creation is a human-approved downstream action."],
        confidence: numberValue(opportunity.confidence, 0.7),
      },
      action: {
        type: "project.create",
    ...
    const execution = {
      ...
      agentDefinitionId: marketAgentId,
      requestedByUserId: user.id,
      ...
      decision: {
        ...
        rationale: "The selected market opportunity must be reviewed by a human before project creation.",
    ...
    const traces = [
      createTraceEvent({
        ...
        sequence: 1,
        type: "decision",
        message: "Project creation drafted",
        payload: {
          opportunityId,
          opportunity,
        } as AgentJsonValue,
      ...
      createTraceEvent({
        ...
        sequence: 2,
        type: "recommendation",
        message: "Project approval requested",
    ...
    const transactionResults = await prisma.$transaction([
      ...
      prisma.agentTask.create({
        data: {
          ...
          title: `Create project from ${stringValue(opportunity.productIdea, "market opportunity")}`,
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to create project approval." },
      { status: 500 },
    );
  }
}
