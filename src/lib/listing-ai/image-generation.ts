...
function buildImagesGenerationsRequest(
...
async function getRemoteImagePayload(url: string) {
  ...
  if (!contentType.startsWith("image/")) {
    throw new Error("生成图片下载失败：上游返回的不是图片。");
  }
...
async function persistGeneratedImages(
  ...
  return Promise.all(
    images.map(async (image) => {
      const payload =
        getBase64ImagePayload(image.url) ?? (await getRemoteImagePayload(image.url));
...
export async function generateListingAiImages(
  body: ImageGeneratorRequest,
  user: CurrentUser,
  scope: WorkspaceScopeInput,
) {
  ...
  if (!referenceImages.length) {
    throw new Error("请至少上传一张六视图或竞品参考图。");
  }
  ...
  try {
    ...
    if (error instanceof Error && error.name === "AbortError") {
      throw new Error(`图片生成请求超时：${settings.timeoutSeconds} 秒内没有返回。`);
    }
    throw error;
  } finally {
  ...
  if (!response.ok) {
    ...
    const sizeHint = errorText.includes("image size must be at least 3686400 pixels")
      ...
      : "";
    throw new Error(`图片生成失败：${response.status}${imageModelHint}${endpointHint}${sizeHint} ${errorText.slice(0, 500)}`);
  }

  let data: ResponsesImageOutput | ImageGenerationsOutput;

  try {
    data = (await response.json()) as ResponsesImageOutput | ImageGenerationsOutput;
  } catch (error) {
    throw new Error(
      error instanceof SyntaxError
        ? "图片生成失败：上游返回的 JSON 不完整。系统已改为请求图片 URL 以减少响应体，请重试。"
        : "图片生成失败：无法解析上游返回结果。",
    );
  }
  ...
  if (!images.length) {
    throw new Error("模型已返回，但没有找到图片结果。请确认模型支持 image_generation，并检查 Base URL/模型名称。");
  }
